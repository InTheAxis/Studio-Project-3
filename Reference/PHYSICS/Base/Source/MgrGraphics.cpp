...
{
	Mtx44 ortho;
	int w = static_cast<int>(Application::GetWindowWidth() * 0.05f);
	int h = static_cast<int>(Application::GetWindowHeight() * 0.05f);
	ortho.SetToOrtho(-w, w, -h, h, -10, 10);
...
void MgrGraphics::RenderTextOnScreen(CompMesh * mesh, std::string text, Color color, float size, float x, float y)
{
	...
	Mtx44 ortho;
	int w = static_cast<int>(Application::GetWindowWidth() * 0.1f);
	int h = static_cast<int>(Application::GetWindowHeight() * 0.1f);
