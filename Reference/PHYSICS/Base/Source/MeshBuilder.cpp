...
CompMesh* MeshBuilder::GenerateSphere(const std::string &meshName, Color color, unsigned numStack, unsigned numSlice, float radius)
{
	...
	for(unsigned stack = 0; stack < numStack + 1; ++stack) //stack //replace with 180 for sphere
	{
		...
		for(unsigned slice = 0; slice < numSlice + 1; ++slice) //slice
		{
			...
			v.texCoord.Set(atan2f(temp.x, temp.z) / (2 * Math::PI) + 0.5f, temp.y * 0.5f + 0.5f);
			vertex_buffer_data.push_back(v);
		}
	}
...
CompMesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string &file_path, std::vector<unsigned char> &heightMap)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for (unsigned z = 0; z < terrainSize; ++z)
	{
		for (unsigned x = 0; x < terrainSize; ++x)
		{
			...
			{
				Vector3 pos(xVal, scaledHeight, zVal);
				Vector3 adjv[4];	
				if (x < terrainSize - 1)
					adjv[0] = Vector3(((x + 1) / (float)terrainSize - 0.5f), (float)heightMap[z * terrainSize + (x + 1)] / SCALE_FACTOR, (z / (float)terrainSize - 0.5f)); //right
				if (x > 0)
					adjv[1] = Vector3(((x - 1) / (float)terrainSize - 0.5f), (float)heightMap[z * terrainSize + (x - 1)] / SCALE_FACTOR, (z / (float)terrainSize - 0.5f)); //left
				if (z < terrainSize - 1)
					adjv[2] = Vector3((x / (float)terrainSize - 0.5f), (float)heightMap[(z + 1) * terrainSize + x] / SCALE_FACTOR, ((z + 1) / (float)terrainSize - 0.5f)); //bottom
				if (z > 0)
					adjv[3] = Vector3((x / (float)terrainSize - 0.5f), (float)heightMap[(z - 1) * terrainSize + x] / SCALE_FACTOR, ((z - 1) / (float)terrainSize - 0.5f)); //top				
