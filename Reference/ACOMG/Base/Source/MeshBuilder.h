...
public:
	...
	static CompMesh* GeneratePlane(const std::string &meshName, Color color, unsigned numRow, unsigned numCol, float tileSize);
	...
	static CompMesh* GenerateTerrain(const std::string &meshName, const std::string &file_path, std::vector<unsigned char> &heightMap, float scaleX = 1, float scaleY = 1, float scaleZ = 1);
	static CompSpriteAnim* GenerateSpriteAnimation(const std::string &meshName, unsigned numRow, unsigned numCol);
