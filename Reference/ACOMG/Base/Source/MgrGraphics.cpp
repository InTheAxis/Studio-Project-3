...
void MgrGraphics::Init()
{
	...
	m_shaders[FBO] = HelperShader::LoadShaders("Shader//fbo.vert", "Shader//fbo.frag");
	m_shaders[DEPTH] = HelperShader::LoadShaders("Shader//depth.vert", "Shader//unlit.frag");	
	m_shaders[BRIGHT] = HelperShader::LoadShaders("Shader//fbo.vert", "Shader//bright.frag");
	m_shaders[BLUR] = HelperShader::LoadShaders("Shader//fbo.vert", "Shader//blur.frag");
	m_shaders[BLOOM] = HelperShader::LoadShaders("Shader//fbo.vert", "Shader//bloom.frag");
	UseShader(LIT);
...
void MgrGraphics::SetUniformDepth()
{
	Mtx44 lightVP, lightView, lightProj;

	if (m_lights[0].type == Light::LIGHT_DIRECTIONAL)
		lightProj.SetToOrtho(-800, 800, -800, 800, -800, 800);
	else
		lightProj.SetToPerspective(90, 1, 0.1f, 200);

	lightView.SetToLookAt(m_lights[0].position.x, m_lights[0].position.y, m_lights[0].position.z, 0, 0, 0, 0, 1, 0);

	lightVP = lightProj * lightView;

	Uniform_("lightVP", lightVP);
	Uniform_("shadowMap", 8);
}

void MgrGraphics::SetUniformBlur(bool horizontal)
{
	glUseProgram(m_shaders[BLUR]);
	HelperShader::SetUniform(m_shaders[BLUR], "horizontal", horizontal);
}
