...
//like lit but with blending multi-texturing
...
void main()
{
	...
	if (texCount > 0) //if valid, i.e. have 2 textures
	{	
		float len =  length(blend.pos - position_modelspace);
		if (!blend.enabled || len > blend.radius)
		{
			baseColor = texture2D(colorTexture[0], texCoord);
		}
		else if (len > blend.radius * blend.cutoff)
		{
			float val = (blend.radius - len) / (blend.radius * (1 - blend.cutoff));
			baseColor = mix(texture2D(colorTexture[0], texCoord), texture2D(colorTexture[1], texCoord), val);
		}
		else
		{
			baseColor = texture2D(colorTexture[1], texCoord);
		}
	}
	else
