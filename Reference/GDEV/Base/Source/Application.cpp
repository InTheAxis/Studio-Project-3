...
void Application::Init()
{
	...
	glfwSetWindowPos(m_window, m_window_width >> 1, m_window_height >> 1);
	...
	glfwSetCursorPos(m_window, m_window_width >> 1, m_window_height >> 1);
...
{
	//if mouse is leaving window, center it
	double mouse_currX, mouse_currY;
	glfwGetCursorPos(m_window, &mouse_currX, &mouse_currY);
	if (Math::FAbs(mouse_currX) >= (m_window_width >> 2) || Math::FAbs(mouse_currY) >= (m_window_height >> 2))
	{
