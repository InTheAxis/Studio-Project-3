...
void RenderHelper::RenderText(Mesh* _mesh, const std::string& _text, Color _color)
{
	...
	for (unsigned i = 0; i < _text.length(); ++i)
	{
		...
		characterSpacing.SetToTranslation((float)i * 0.65f, 0.0f, 0.01f * i); // 1.0f is the spacing of each character, you may change this value
	...
	// Reset the text color to white, so it will not affect the rendering for other entities
	Color _colorReset(1.0f, 1.0f, 1.0f);
	currProg->UpdateVector3("textColor", &_colorReset.r);
}
