...
void ParticleList::Init(CompSpriteAnim* mesh)
{	
	for (long i = 0; i < 10; ++i)
	{
	...
	numPerFrame = 1;
	m_emissionPointMax = Vector3(100, 220, -1000);
	m_emissionPointMin = Vector3(-100, 220, -1100);
	m_velMax = Vector3(10, 0, 10);
	m_velMin = Vector3(-10, 0, -10);
	...
	//numPerFrame = 10;
	//m_emissionPointMax = Vector3(0, 220, 0);
	//m_emissionPointMin = Vector3(0, 220, 0);
	//m_velMax = Vector3(15, 140, 0);
	//m_velMin = Vector3(10, 90, 0);
	//m_rotateSpeedMax = 10;
	//m_rotateSpeedMin = -10;
	//m_gravity = Vector3(0, -100, 0);
	//m_lifeSpan = 4.f;
	//m_particleScale = 5;
}
...
void ParticleList::Render(MS* modelStackRef, CameraBase* cameraRef)
{	
	for (auto it = m_arr.rbegin(); it != m_arr.rend(); ++it)
	{
		...
		MgrGraphics::Instance().RenderSpriteAnim(mesh, MgrGraphics::UNLIT);
