...
{	
	...
	// Use m_fElapsedTimeBeforeUpdate to do the loop control

	m_fElapsedTimeBeforeUpdate += dt;
	if (m_fElapsedTimeBeforeUpdate > 5)
	{		
		m_fElapsedTimeBeforeUpdate = 0;
		// Put the A.I. logic here
		position += view * (float)m_dSpeed * (float)dt;
		UpdateTargetRightUp(target, right, up);
	}

	// End of RealTime Loop control
}
...
void CEnemy3D::UpdateTargetRightUp(Vector3 & target, Vector3 & right, Vector3 & up)
{
