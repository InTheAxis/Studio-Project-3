...
public:
	...
	//update player tile pos
	void UpdatePlayerTile(void);
	// Update tile offsets
	void UpdateTileOffset(void);
	// Update side movements
	void UpdateSideMovements(void);
	// Update vert movements
	void UpdateVertMovements(void);
	//update wrap around for screen based
	void UpdateWrapAround(void);
	//update for climbing ladders
	void UpdateClimbLadder(bool dir);
	...
	float playerTile_X, playerTile_Y;	
	Vector3 position;
	...
	*/
	...
	double Token2Int(const string token);
	bool Token2Bool(const string token);

	// Check for collision with CGoodies instances, returns collided goodie
	CGoodies* CollideWithGoodies();
